        ...
        <div className="rounded-2xl p-6 sm:p-8 animate-in" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" placeholder="Jean" value={form.prenom}
                    onChange={e => updateForm("prenom", e.target.value)} className="input-field" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1.5">Nom</label>
                  <input type="text" placeholder="Dupont" value={form.nom}
                    onChange={e => updateForm("nom", e.target.value)} className="input-field" />
                </div>
              </div>

              <div className="mt-4">
                <label className="block text-sm font-semibold mb-1.5">Adresse e-mail</label>
                <input type="email" placeholder="jean.dupont@email.com" value={form.email}
                  onChange={e => updateForm("email", e.target.value)} className="input-field" />
              </div>

              <div className="mt-4">
                <label className="block text-sm font-semibold mb-1.5">Téléphone</label>
                <input type="tel" placeholder="06 12 34 56 78" value={form.telephone}
                  onChange={e => updateForm("telephone", e.target.value)} className="input-field" />
              </div>
              ...
              <div className="mt-4">
                ...
                <input type="password" placeholder="6 caractères minimum" value={form.password}
                  onChange={e => updateForm("password", e.target.value)} className="input-field" />
              </div>
            </div>
          )}
          ...
          {etape === 2 && (
            <div>
              ...
              <p className="text-sm mb-6" style={{ color: "var(--color-text-secondary)" }}>
                Ces informations seront visibles par les élèves sur votre profil.
              </p>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {DIPLOMES.map(d => (
                    <div key={d.value} onClick={() => updateForm("diplome", d.value)}
                      className={`chip ${form.diplome === d.value ? "chip-active" : ""}`}>
                      {d.label}
                    </div>
                  ))}
                </div>
              ...
              <div className="mb-6">
                ...
                <input type="number" placeholder="Ex : 5" min="0" max="50" value={form.experienceAnnees}
                  onChange={e => updateForm("experienceAnnees", e.target.value)} className="input-field"
                  style={{ maxWidth: 160 }} />
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {TYPES_BOITE.map(tb => (
                    <div key={tb.value} onClick={() => updateForm("typeBoite", tb.value)}
              ...
              <div className="mb-6">
                ...
                <p className="text-xs mb-3" style={{ color: "var(--color-text-muted)" }}>
                  Vous conservez 85 % de ce montant. Easy Drive prélève 15 % de commission.
                </p>
                <input type="range" min="25" max="80" step="5" value={form.tarifHoraire}
                  onChange={e => updateForm("tarifHoraire", e.target.value)}
                  className="w-full accent-[#00B37D]" />
                <div className="flex justify-between text-xs mt-1" style={{ color: "var(--color-text-muted)" }}>
                  <span>25 €</span><span>80 €</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {SPECIALITES.map(s => (
                    <div key={s.value} onClick={() => toggleArray("specialites", s.value)}
                      className={`chip ${form.specialites.includes(s.value) ? "chip-active" : ""}`}>
                      {s.label}
                    </div>
                  ))}
                </div>
          ...
          {/* ─────── ÉTAPE 3 : Zone et disponibilités ─────── */}
          {etape === 3 && (
            <div>
              <h2 className="text-xl font-bold mb-1" style={{ fontFamily: "var(--font-display)" }}>
                Zone et disponibilités
              </h2>
              <p className="text-sm mb-6" style={{ color: "var(--color-text-secondary)" }}>
                Dernière étape ! Indiquez où et quand vous êtes disponible.
              </p>
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {ZONES_IDF.map(z => (
                    <div key={z.value} onClick={() => updateForm("zone", z.value)}
                      className={`chip ${form.zone === z.value ? "chip-active" : ""}`}>
                      {z.label}
                    </div>
                  ))}
                </div>
              ...
              <div className="mb-5">
                <label className="block text-sm font-semibold mb-1.5">Ville ou point de départ habituel</label>
              ...
              <div>
                ...
                <textarea
                  placeholder="Parlez de votre approche pédagogique, de ce qui vous différencie…"
                  value={form.bio}
